...
export const formulaData: Formula[] = [
  ...
  // Additional formulas identified in audit
  { id: 'km-estimator', name: 'Kaplan-Meier Estimator', tex: '\\hat{S}(t) = \\prod_{t_j \\le t} \\left(1 - \\frac{d_j}{n_j}\\right)', description: 'Non-parametric survival estimate. At each event time, multiply the running survival by the fraction who survived that moment. d_j = deaths at time j, n_j = at risk.', chapter: 'ch6', category: 'Survival Analysis' },
  { id: 'nb-variance', name: 'Negative Binomial Variance', tex: '\\text{Var}(Y) = \\mu + \\alpha\\mu^2', description: 'Variance of the negative binomial distribution. The extra term alpha*mu^2 accommodates overdispersion. When alpha=0, this reduces to Poisson.', chapter: 'ch5', category: 'Count Data' },
  { id: 'hausman-test-stat', name: 'Hausman Test Statistic', tex: 'H = (\\hat{\\beta}_{IV} - \\hat{\\beta}_{OLS})\'[\\hat{V}_{IV} - \\hat{V}_{OLS}]^{-1}(\\hat{\\beta}_{IV} - \\hat{\\beta}_{OLS}) \\sim \\chi^2_k', description: 'Tests whether the difference between IV and OLS estimates is systematic. Significant H rejects exogeneity, justifying the IV approach.', chapter: 'ch8', category: 'Instrumental Variables' },
  { id: 'ovb-formula', name: 'Omitted Variable Bias', tex: '\\text{Bias}(\\hat{\\beta}_1) = \\beta_2 \\cdot \\frac{\\text{Cov}(X_1, X_2)}{\\text{Var}(X_1)}', description: 'When a relevant variable X2 is omitted, the coefficient on X1 absorbs the effect of X2 scaled by their correlation. Direction depends on signs of beta2 and the correlation.', chapter: 'ch8', category: 'Instrumental Variables' },
  { id: 'bayes-theorem', name: 'Bayes Theorem (Classification)', tex: 'P(C|X) = \\frac{P(X|C) \\cdot P(C)}{P(X)}', description: 'Foundation of Naive Bayes classification. The posterior probability of class C given features X is proportional to the likelihood times the prior.', chapter: 'ch10', category: 'Text Mining' },
  { id: 'f1-score', name: 'F1 Score', tex: 'F_1 = 2 \\cdot \\frac{\\text{Precision} \\cdot \\text{Recall}}{\\text{Precision} + \\text{Recall}}', description: 'Harmonic mean of precision and recall. Balances the trade-off between false positives and false negatives. Ranges from 0 to 1.', chapter: 'ch3', category: 'Binary Response' },
]
